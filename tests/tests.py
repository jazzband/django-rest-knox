...
knox_settings.defaults["MIN_REFRESH_TOKEN_ISSUE_INTERVAL"] = timedelta(seconds=-1)

refresh_token_limit_per_user = knox_settings.defaults.copy()
refresh_token_limit_per_user["REFRESH_TOKEN_LIMIT_PER_USER"] = 10

min_refresh_issue_interval_one_hour = knox_settings.defaults.copy()
min_refresh_issue_interval_one_hour["MIN_REFRESH_TOKEN_ISSUE_INTERVAL"] = timedelta(
    hours=1
)
...
class AuthTestCase(TestCase):
    ...
    def test_logout_deletes_keys(self):
        ...
        if knox_settings.ENABLE_REFRESH_TOKEN:
            self.assertEqual(
                RefreshFamily.objects.count(),
                1,
                "other tokens should remain after logout",
            )

            self.assertEqual(
                AuthRefreshToken.objects.count(),
                1,
                "other tokens should remain after logout",
            )
    ...
    def test_expiry_present_also_when_none(self):
        with override_settings(REST_KNOX=token_no_expiration_knox):
            ...
            if knox_settings.ENABLE_REFRESH_TOKEN:
                ...
                self.assertEqual(response.data["refresh_token_expiry"], None)
    ...
    def test_invalid_refresh_short_token_returns_400(self):
        if knox_settings.ENABLE_REFRESH_TOKEN:
            ...
            # valid
            response = self.client.post(
                url, {"refresh_token": refresh_token[8]}, format="json"
            )
    ...
    def test_invalid_refresh_longer_token_returns_400(self):
        if knox_settings.ENABLE_REFRESH_TOKEN:
            ...
            # valid
            response = self.client.post(
                url, {"refresh_token": refresh_token + "x"}, format="json"
            )
        ...
        if knox_settings.ENABLE_REFRESH_TOKEN:
            ...
            for i in range(10):
                if i == 0:
                    url = reverse("knox_refresh")
                    response = self.client.post(
                        url, {"refresh_token": first}, format="json"
                    )
                    refresh_token = response.data["refresh_token"]
                else:
                    response = self.client.post(
    ...
    def test_refresh_before_min_refresh_issue_interval(self):
        if knox_settings.ENABLE_REFRESH_TOKEN:
            original_time = datetime.now()
            ttl = timedelta(hours=1)
            with override_settings(REST_KNOX=min_refresh_issue_interval_one_hour):
                reload(views)
                with freeze_time(original_time):
                    reload(views)
                    url = reverse("knox_login")
                ...
                one_second_before_min = original_time + ttl - timedelta(seconds=1)

                with freeze_time(one_second_before_min):
                    reload(views)
                    url = reverse("knox_refresh")
                    response = self.client.post(
                        url, {"refresh_token": refresh_token}, format="json"
    ...
    def test_refresh_after_min_refresh_issue_interval(self):
        if knox_settings.ENABLE_REFRESH_TOKEN:
            original_time = datetime.now()
            ...
            with override_settings(REST_KNOX=min_refresh_issue_interval_one_hour):
                ...
                one_second_after_expiry = original_time + ttl + timedelta(seconds=1)

                with freeze_time(one_second_after_expiry):
                    reload(views)
                    url = reverse("knox_refresh")
                    response = self.client.post(
                        url, {"refresh_token": refresh_token}, format="json"

...
class AbstractAuthRefreshToken(models.Model):
    ...
    user = models.ForeignKey(User, null=False, blank=False,
                             related_name='refresh_token_set', on_delete=models.CASCADE)
    created = models.DateTimeField(auto_now_add=True)
    expiry = models.DateTimeField(null=True, blank=True)
...
class RefreshFamilyManager(models.Manager):
        def create(self, user,token,refresh_token,parent=None):
            instance = super(RefreshFamilyManager, self).create(
                ...
                parent=parent[:CONSTANTS.TOKEN_KEY_LENGTH] if parent else refresh_token[:CONSTANTS.TOKEN_KEY_LENGTH],
...
class AbstractRefreshFamily(models.Model):
    ...
    refresh_token = models.CharField(max_length=CONSTANTS.TOKEN_KEY_LENGTH, primary_key=True,db_index=True)
    created = models.DateTimeField(auto_now_add=True)
    user = models.ForeignKey(User, null=False, blank=False,
                             related_name='refresh_family_set', on_delete=models.CASCADE)

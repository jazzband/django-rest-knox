...
class RefreshTokenView(LoginView,TokenAuthentication):
    ...
    def add_refresh_token(self, data, token, parent):
        refresh_instance,refresh_token = self.create_refresh_token()
        self.create_refresh_family(parent=parent,refresh_token=refresh_token,token=token) 
        data['refresh_token'] = refresh_token
        data['refresh_token_expiry'] = self.format_expiry_datetime(refresh_instance.expiry)
        return data

    def validate_member(self,refresh_token):
        member = self.refresh_family_model.objects.filter(refresh_token=refresh_token[:CONSTANTS.TOKEN_KEY_LENGTH]).first()
        if member is None:
            return None,None
    ...
    def delete_old_tokens(self,family):
        for token in family:
            ...
            self.refresh_token_model.objects.filter(token_key=token.refresh_token).delete()
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            for member in family:
                ...
                self.refresh_token_model.objects.filter(token_key=member.refresh_token).delete()
            family.delete() 
            raise exceptions.AuthenticationFailed(_('Invalid token.'))
